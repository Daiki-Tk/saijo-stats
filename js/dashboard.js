...
/* =====================================================
   データ概要ボックス（summary.md の内容を組み込み）
   ===================================================== */

// 各セクションに表示するデータ概要テキスト（summary.md より）
const SECTION_SUMMARIES = {
  pop: '2011年から2026年にかけて総人口が114,384人から101,711人へ約10%減少し、特に若年層（15歳未満）の割合が13.5%から11.1%へ低下している。一方、老年人口（65歳以上）の割合は26.4%から33.8%へ急増し、高齢化が顕著に進行している。世帯数は約50,000世帯で安定的に推移しているが、1世帯あたりの人口が減少していることから、単身世帯や小規模世帯の増加が示唆される。',
  ind: '製造業事業所数は2011年の282から2021年の240へと約15%減少し、産業の集約化が進んでいます。一方、従業者数は約9,000〜10,000人で比較的安定しており、出荷額も年間70〜96万単位で推移しているなど、規模の縮小傾向は見られません。GDPと1人当たり所得は変動幅があるものの、全体的には緩やかな成長傾向にあり、地域経済の基盤として機能していることが伺えます。',
  agr: '水稲作付面積は2011年の3,500ヘクタールから2024年の2,830ヘクタールへと継続的に減少しており、約19%の縮小を示しています。大麦作付面積は880ヘクタール前後で推移する一方、大麦の単収は1,900〜3,690kg/haと変動が大きく、2020年にピークを記録した後、最近年は低下傾向を見せています。民有林面積は約27,800〜28,200ヘクタール程度で安定的に推移しており、水稲の単収低下と同様に農業生産性の課題が懸念されます。',
  fin: '2011年から2024年のデータを分析すると、財政収入（歳入）は45,264から59,661百万円へと約31%増加した一方、支出（歳出）も同程度の増加を示しており、財政が安定的に拡大しています。税収全体は15,429〜16,960百万円の範囲で推移し比較的安定していますが、特に2024年は固定資産税が大きく増加しています。歳出面では教育予算が2024年に8,317百万円と過去最高水準に達するなど、投資の重点が教育分野に傾斜していることが特徴です。',
  edu: '小学校・中学校・高等学校の児童生徒数は2011年から継続して減少傾向にあり、2011年の約12,000人から2025年には約8,600人へと大幅に縮小しています。一方、こども医療費助成対象者数は2024年に14,000人を超えるピークに達するなど、児童福祉への支出は増加傾向を示しています。保育所入所児童数は2,000人前後で推移しており、教育から福祉へのシフトが進行していることが窺えます。',
  obs: '2020年のコロナパンデミック時に各観光施設の利用者数が大幅に低下し、特に最も落ち込んだ2021年から緩やかな回復傾向を見せています。2024年の観光客数は過去最高水準に達し、石鎚ふれあいの里は8,316人と2011年比で約43%増加しています。図書館貸出冊数も同様のパターンを示しており、コロナからの観光・文化活動の回復が顕著です。'
};

// データ概要ボックスを各セクションの冒頭に挿入する
function injectSummaryBoxes() {
  Object.entries(SECTION_SUMMARIES).forEach(([id, text]) => {
    const section = document.getElementById('s-' + id);
    if (!section) return;

    // <details> 要素を生成（デフォルトは閉じた状態）
    const details = document.createElement('details');
    details.className = 'summary-box';
    details.innerHTML =
      '<summary>📊 データ概要</summary>' +
...
async function init() {
  try {
    ...
    injectSummaryBoxes();  // データ概要ボックスを各セクションに挿入
